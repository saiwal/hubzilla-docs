...
```php
public setAuthorizationCodes(mixed $authorization_codes): mixed
```
...
```php
public getUserClaims(mixed $user_id, mixed $claims): array
```
...
```php
protected getUserClaim(mixed $claim, mixed $userDetails): mixed
```
...
```php
public setRefreshTokens(mixed $refresh_tokens): mixed
```
...
```php
public setAccessToken(mixed $access_token, mixed $client_id, mixed $user_id, mixed $expires, mixed $scope = null, mixed $id_token = null): mixed
```
